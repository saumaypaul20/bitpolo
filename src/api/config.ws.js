        ...
        // Event handlers
            switch(result.method){
                ...
                case "depth.update":
                    ...
                    if(state.marketReducer.activeTradePair&& state.marketReducer.activeTradePair === res.params[2]){
...
export const emitMarketListEvent = (marketPairs) =>{
    ...
    socket.emit("message", {"id": Math.floor(Math.random() * 9000000), "method" : "state.subscribe", "params" : marketPairs });
}
...
export const emitUnsubMarketListEvent = (marketPairs) =>{
    ...
    //console.log("soceklt mareket_paors",marketPairs)
    socket.emit("message", {"id": Math.floor(Math.random() * 9000000), "method" : "state.unsubscribe", "params" : marketPairs });
}
...
export const emitMarketDealsEvent = (marketPairs) =>{
    ...
    // console.log("soceklt mareket_paors",marketPairs)
    socket.emit("message", {"id": Math.floor(Math.random() * 9000000), "method" : "deals.subscribe", "params" : [marketPairs] });
}
...
export const emitKlineSubscribeEvent = (pair, lastpair,interval=900) =>{
    ...
    socket.emit("message", {"id": Math.floor(Math.random() * 9000000), "method" : "kline.subscribe", "params" : [pair, interval] });
    //
}
export const emitKlineQuerySubscribeEvent = (pair, interval=900) =>{
    ...
    console.log(JSON.stringify({"id": Math.floor(Math.random() * 6000000), "method" : "kline.query", "params" : [pair, Math.floor(new Date().getTime()/1000) - 3*24*60*60*1000, Math.floor(new Date().getTime()/1000),interval] }))
    console.log("^^^^^^^^^^^^^^^^^^^$$$$$$$$$$$**************)))))))))))))))###########")
    console.log("^^^^^^^^^^^^^^^^^^^$$$$$$$$$$$**************)))))))))))))))###########")
    // console.log("pair",((new Date().getTime() )- 3*24*60*60*1000),(new Date().getTime()))
    socket.emit("message", {"id": Math.floor(Math.random() * 6000000), "method" : "kline.query", "params" : [pair, Math.floor(new Date().getTime()/1000) - 3*24*60*60*1000, Math.floor(new Date().getTime()/1000),interval] });
}
...
export const emitDepthSubscribeEvent = (lastpair, newpair) =>{
 ...
 if(!newpair){
    ...
    console.log(JSON.stringify({"id": Math.floor(Math.random() * 90000000), "method" : "depth.subscribe", "params" : [newpair, 9,"0"] }))
...
export const emitDepthUnsubscribeEvent = (lastpair) =>{
    ...
    socket.emit("message", {"id": Math.floor(Math.random() * 9000000), "method" : "depth.unsubscribe", "params" : [lastpair] });
}

export const emitKlineUnsubscribeEvent = (lastpair) =>{
    // alert("called unsb depths")
    socket.emit("message", {"id": Math.floor(Math.random() * 9000000), "method" : "kline.unsubscribe", "params" : [lastpair] });
}
