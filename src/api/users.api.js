...
export const resetPasswordHashValidation = async hash => {
  return new Promise(async (resolve, reject) => {
    ...
    let res = await fetchApi(
      `${REST.USERS.RESET_PASSWORD}/${hash}`,
      'GET',
      null,
      200,
      headers,
...
export const resetPassword = async (payload, toPassHeader) => {
  return new Promise(async (resolve, reject) => {
    ...
    let password_confirmation = await encryptValue(
      payload.data.attributes.password_confirmation,
    );
...
export const getMarketByPair = async (pair, toPassHeader) => {
  return new Promise(async (resolve, reject) => {
    ...
    let res = await fetchApi(
      `${REST.USERS.GET_MARKET_BY_PAIR}/${pair}`,
      'GET',
      null,
      200,
      headers,
