...
export default function DepthChart(props) {
  const rawasks = useSelector(
    state => state.depthSubsReducer.data?.params[1]?.asks,
    equalityFnDepths,
  );
  const rawbids = useSelector(
    state => state.depthSubsReducer.data?.params[1]?.bids,
    equalityFnDepths,
  );
  ...
  function processData(data, side) {
    ...
    return data
      ...
      .reduce((arr, item, idx) => {
        ...
        const bid = [Number(item.p), bidsAmtCount];
  ...
  if (bidsD.length === 0 || asksD.length === 0)
    return <ActivityIndicator size="large" color={Colors.white} />;
  return <HighChart height={props.height} asks={asks} bids={bids} />;
}
