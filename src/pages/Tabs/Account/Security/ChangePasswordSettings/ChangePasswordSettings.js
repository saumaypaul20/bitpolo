...
import { generateOtp, resetPasswordHashValidation } from '../../../../../api/users.api'
...
const ChangePasswordSettings = (props) => {
    ...
    const onSubmit = async () =>{
        if(!newPwd || newPwd.trim().length == 0 ){ alert("Enter the new password"); return}
        if(!oldPwd || oldPwd.trim().length == 0 ){ alert("Enter the old password"); return}
        if(!confirm || confirm.trim().length == 0){ alert("Re-Enter the  new password"); return}
        if(confirm.trim() !== newPwd.trim()){ alert("New Password doesn't match!"); return}

        switch(props.route.params.google_auth){
            
            case true:
                navigation.navigate(screenNames.GOOGLE_VERIFICATION_CODE,{ id:user.id ,passwords:{old_password: oldPwd, password: newPwd, c_password: confirm} , type: props.route.params.type})
                break
            default:
                ...
                if(res.status){
                    ...
                    if(ress.status){
                        navigation.navigate(screenNames.VERIFY_EMAIL, {validated_data: res.data.data, passwords: {old_password: oldPwd, password: newPwd, c_password: confirm, type:props.route.params.type}})
                    }else{
