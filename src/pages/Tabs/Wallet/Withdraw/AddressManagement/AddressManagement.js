...
import {
  ...
  updateWithdrawAddresses,
  deleteWithdrawAddresses,
} from '../../../../../api/wallet.api';
  ...
  const onAddWithdrawAddress = async () => {
    ...
    if (user.attributes.google_auth) {
      ...
      let body1 = {
        ...
        data: {
          id: user.id,
          attributes: {type: 'withdraw address'},
        },
      };
      let toPassHeader = {
          ...
          <Content contentContainerStyle={{flexGrow: 1}}>
              ...
              <View
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  ...
                  <Image
                    ...
                    style={{width: 14, marginLeft: 5, height: 14}}
                    resizeMode="contain"
                  />
                </View>
              </View>
            ...
            <View
              ...
              <QuickButtons
                ...
                onPress={() => modifyBeingWhitelist(queue[0])}
              />
              <QuickButtons
                disabled={queue.length === 0}
              ...
              <QuickButtons
                disabled={queue.length === 0}
                label={'Delete'}
                onPress={() => modifyBeingWhitelist(queue[0], true)}
              />
            </View>
            <View
              style={{
              ...
              {items.length > 0 &&
                items.map((item, index) => {
                  return (
                    <ListItem
                      active={
                        queue.length > 0 && queue.find(i => item._id === i._id)
                          ? true
                          : false
                      }
                      item={item}
                      key={item._id}
                      onPress={() => handleWhitelist(item)}
                      onStarPress={() => modifyBeingWhitelist(item)}
                      isWhitelist={
                        whitelist.length > 0 &&
                        whitelist?.find(i => i._id == item._id)
                      }
                    />
                  );
                })}
            ...
            <Modal
              ...
              <BPText style={{marginVertical: 10}}>
    ...
    <View
      ...
      <View style={{flex: 0.5, justifyContent: 'center', alignItems: 'center'}}>
        <TouchableOpacity
          ...
          {active && (
            <View
              style={{backgroundColor: Colors.white, width: 5, height: 5}}
            />
          )}
        </TouchableOpacity>
      </View>
      ...
      <View style={{flex: 2}}>
        <TouchableOpacity onPress={() => copyText(item.address, 'Address')}>
          ...
          <BPText numberOfLines={1} style={{fontSize: 12}}>
            {item.address}
          </BPText>
        </TouchableOpacity>
      </View>
      <View style={{flex: 1}}>
        <TouchableOpacity
          ...
          <Image
            source={item.is_whitelist ? Images.star_icon : Images.star_active}
