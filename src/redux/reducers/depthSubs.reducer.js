    ...
    case TYPES.ADD_DEPTH_SUBS_DATA:
      ...
      if (!state?.data) {
        ...
        if (state?.data?.params[2] === action?.payload?.params[2]) {
          let diffasks = equalityFnDepths(
            action.payload.params[1].asks,
            state.data.params[1].asks,
          );
          let diffbids = equalityFnDepths(
            action.payload.params[1].bids,
            state.data.params[1].bids,
          );
